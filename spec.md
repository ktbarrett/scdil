...
Newlines include `\n`, `\r\n` and `\r` characters.
Newlines are generated by the lexer, but are ignored while parsing composites.
...
**Parse Rules**
```
sequence = "[" nl* (value nl* "," nl*)* (value nl* ","? nl*)? "]"
```
...
**Parse Rules**
```
mapping = "{" nl* (value nl* ":" nl* value nl* "," nl*)* (value nl* ":" nl* value nl* ","? nl*)? "}"
```
...
**Parse Rules**
```
block_sequence(N, M) where M>N = ("-"(N) (value nl+ | nl* block(M)))+
```
...
**Parse Rules**
```
block_mapping(N, M) where M>N = ((name | string)(N) ":" (value nl+ | nl* block(M)))+
...
literal_lines(N) = ("|"(N) character* nl+)+
folded_lines(N) = (">"(N) character* nl+)+
escaped_literal_lines(N) = ("\|"(N) (escape | character)* nl+)+
escaped_folded_lines(N) = ("\>"(N) (escape | character)* nl+)+
```
